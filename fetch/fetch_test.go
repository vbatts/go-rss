...
func init() {
	go func() {
		http.HandleFunc("/announce.xml", func(w http.ResponseWriter, r *http.Request) {
			file, err := os.Open("../announce.xml")
			if err != nil {
				http.NotFound(w, r)
			}
			defer file.Close()
...
func TestFetchAnnounce(t *testing.T) {
	r, err := FetchRss("http://localhost:8080/announce.xml")
	if err != nil {
		t.Fatalf("Failed to connect!, %s", err)
	}
	exp_str := "announce July 2013 Archive"
